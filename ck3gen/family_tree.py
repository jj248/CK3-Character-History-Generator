...
from ck3gen.paths import TREE_OUTPUT_DIR
...
class FamilyTree:
    ...
    def __init__(self, character_file, title_file, config):
        ...
        self.config = config
        self.graphLook = self.config['initialization'].get('treeGeneration', 'LR')
    ...
    def render_trees(self):
        """Render the family trees to files in the canonical tree output directory."""
        TREE_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

        for dynasty, graph in self.graphs.items():
            filename = str(TREE_OUTPUT_DIR / f"family_tree_{dynasty}")
