...
class Simulation:
    ...
    def character_death_check(self, character):
        ...
        for event in self.config.get('initialization', {}).get('events', []):
            if birth_year >= event.get("startYear") and birth_year <= event.get("endYear") and age >= event.get("characterAgeStart") and age <= event.get("characterAgeEnd"):
                # deathMultiplier > 1 raises lethality; multiply into the mortality rate
                # and clamp to [0.0, 1.0] so it stays a valid probability.
        ...
        return random_var < mortality_rate
    ...
    def _backfill_pre_simulation_events(self, min_year: int) -> None:
        """
        Emit childhood-trait (age 3) and personality-trait (age 16) events for any
        character that was born before minYear and therefore never passed through the
        year-by-year handlers for those milestones.
        """
        for character in self.all_characters:
            age_at_start = min_year - character.birth_year
            if age_at_start <= 3:
                continue  # Will be handled normally by the loop

            # Age-3 childhood trait — emit if the character has no trait events yet
            has_childhood_event = any(
                "trait =" in detail
                for _, detail in character.events
                if detail.strip().startswith("trait =")
            )
            if not has_childhood_event:
                skill = character.education_skill or "diplomacy"
                choices = self.childhood_by_education.get(skill, ["charming", "curious"])
                trait = random.choice(choices)
                date = f"{character.birth_year + 3}.{character.birth_month:02d}.{character.birth_day:02d}"
                character.add_event(date, f"trait = {trait}")

            # Age-16 personality traits — emit if not yet assigned
            if age_at_start > 16 and not character.personality_traits:
                character.assign_personality_traits(self.config['skills_and_traits']['personalityTraits'])
                date = f"{character.birth_year + 16}.{character.birth_month:02d}.{character.birth_day:02d}"
                detail_lines = [f"trait = {t}" for t in character.personality_traits]
                event_detail = "\n    ".join(detail_lines)
                character.add_event(date, event_detail)
    ...
    def run_simulation(self):
        ...
        # Emit childhood/personality events for characters seeded before minYear
    ...
    def export_characters(self, output_filename="family_history.txt"):
        # Wire language rules into the Character class before any format_for_export() call.
        # config_loader builds these; they live on the config dict under initialization.dynasties.
        from ck3gen.config_loader import ConfigLoader
        # Re-use already-parsed data rather than re-loading files: build the rules inline
        # from whatever is in self.config so we don't need a second ConfigLoader instance.
        language_rules: dict = {}
        for dynasty in self.config.get("initialization", {}).get("dynasties", []):
            dynasty_id = dynasty.get("dynastyID", "")
            rules = []
        ...
        # Use the canonical output directory from ck3gen.paths
